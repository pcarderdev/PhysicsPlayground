...
function two_body_dynamics!(du, u, p, t)
    pos1 = SVector(u[1], u[2], u[3])
    pos2 = SVector(u[4], u[5], u[6])
    vel1 = SVector(u[7], u[8], u[9])
    vel2 = SVector(u[10], u[11], u[12])
...
function plot_simulation(sol)
    ...
    # Plot trajectories
    plot!(x1, y1, z1, label="Particle 1", linewidth=2, color=:blue)
    plot!(x2, y2, z2, label="Particle 2", linewidth=2, color=:red)

    # Mark starting positions
    scatter!([x1[1]], [y1[1]], [z1[1]], label="Start 1", color=:blue, markersize=8)
    scatter!([x2[1]], [y2[1]], [z2[1]], label="Start 2", color=:red, markersize=8)

    # Mark final positions
    scatter!([x1[end]], [y1[end]], [z1[end]], label="End 1", color=:lightblue, markersize=8)
    scatter!([x2[end]], [y2[end]], [z2[end]], label="End 2", color=:pink, markersize=8)
end
